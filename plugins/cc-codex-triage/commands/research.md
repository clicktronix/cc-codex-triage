---
description: Research a technical question with Codex using current primary sources and repository evidence. Use for comparing approaches, checking documentation and investigating unfamiliar systems before deciding or implementing.
...
1. Use the user's question and existing task constraints. Infer routine context from
   the repository; ask only for a missing decision that materially changes the study.
   `--thread` selects a conversation; otherwise use the branch-scoped research name.
...
2. Compose `$PROMPT` with the question, intended decision, relevant repository paths,
   ...
   Read applicable repository instructions and relevant code to establish context.
...
3. Dispatch using the existing wrapper. The live-search setting is requested explicitly
   ...
   If external tools are unavailable, return useful repository findings with that limit;
...
4. Check consequential claims and citations before relying on them. Present a concise
   ...
   log. For a requested report, the owner writes it to the repository's established
