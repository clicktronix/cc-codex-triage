...
3. If any `WARNING` line appears, briefly tell the user what to do:
   ...
   - *the log's last verdict reads APPROVE but the required gate did not accept it* → Codex decorated the verdict (`## APPROVE`, `**APPROVE**`); the required gate takes the bare token on its own final line, and the advisory verdict column is not the gate. Follow the recovery the same block prints, which depends on the status: re-run the round for a recoverable one, but a `CAP_REACHED` / `DIVERGED` hard stop is not retryable, and a `STALE` round names its recorded reason rather than assuming the candidate moved.
