...
3. If any `WARNING` line appears, briefly tell the user what to do:
   ...
   - *the log's last verdict reads APPROVE but the required gate did not accept it* → the informational parser saw a verdict, but the machine gate has not accepted it. A completed round may have decorated the token (`## APPROVE`, `**APPROVE**`); under `PENDING`, it may instead be the current round's bare reply already in the log but not yet recorded. Follow the recovery printed for that exact status: re-run only a recoverable completed round, never a live claim or `CAP_REACHED` / `DIVERGED` hard stop; a `STALE` round names its recorded reason rather than assuming the candidate moved.
