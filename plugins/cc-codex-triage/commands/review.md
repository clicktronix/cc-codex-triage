...
7. Show Codex's reply verbatim (except `--json` — rendered per step 6 instead). Exit code 4 (resume failed) → ask the user before `--new`, per skill. Exit code 5 / porcelain warning → surface the diff (Codex touched files).
   ...
   The owning workflow must require this exact marker and compare `head` to its candidate HEAD. Any nonzero check or missing/mismatched marker means no approval.

   A workflow that re-verifies later, outside this command, cannot use `${CLAUDE_PLUGIN_ROOT}` — by then it names the owning plugin, not this one. Resolve this plugin from the installed-plugin manifest (`~/.claude/plugins/installed_plugins.json`, key `cc-codex-triage@cc-codex-triage`, entries whose `scope` is `user` or whose `projectPath` is the repository), and accept only a root that carries both `commands/review.md` and `scripts/review-state.sh` with the `CC_CODEX_REQUIRED_REVIEW APPROVE` contract in each — the same test the consumer's own prerequisite check applies, and take the first match with project- and local-scoped installs ordered before user-scoped ones. Those two rules are what keep a preflight and a later gate from qualifying different installations; they do not make it impossible for a caller to point the two at different manifests, and nothing here should be read as claiming otherwise. Run `review-state.sh check` there with `CC_CODEX_STATE_DIR` and `CC_CODEX_GATE_DIR` cleared: those are test hooks, and a gate that lets its caller redirect the state it reads is not a gate.
...
8. **Validate each finding against the code** — per the skill's "validating inbound Codex findings" rule. Read the cited site *and its consumers*, check for a documented reason the current code stands, confirm the suggested fix would not regress, and classify each finding valid / borderline / invalid / outdated. Reject invalid/outdated ones via `/reply <THREAD>` (pass the SAME thread explicitly — `/reply`'s default is `review-<branch-slug>`, aligned with this command's, but an explicit name is still safer when several review threads exist) with the concrete file:line that refutes them; surface borderline/architectural ones to the user. **Never apply a finding you believe is wrong just to reach APPROVE.**
   ...
   In `--required` mode, stop here on every `REQUEST_CHANGES`: do not edit files, stage, commit, or run a repair loop inside this command. Return the validated findings without an approval marker. The owning workflow must then resolve every blocker and invoke this same required thread again. A valid finding goes through the explicit fix transition: implement it, test it, and commit a new clean candidate before re-review. A finding refuted with concrete evidence or explicitly deferred through the findings-ledger flow does not justify a fake code change or empty commit: send that disposition back on the same thread and re-run required review against the same immutable candidate. In both paths, the earlier `REQUEST_CHANGES` remains non-approval evidence and a fresh round must earn `APPROVE`. This separation is mandatory even when the fix looks trivial.
