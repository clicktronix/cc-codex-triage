...
2. Resolve `STATE_DIR` with `state-dir.sh`, then decide **initial vs resume**: it is a resume if `$STATE_DIR/<THREAD>.id` exists. Do NOT hand-compute a round number — the driver stamps `round=N` in the log header.
...
- Thread state is in the repository common Git directory reported by `state-dir.sh`. Force-reset: `/thread-new <thread>`.
