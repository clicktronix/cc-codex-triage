...
argument-hint: "[--rounds N] [--thread <name>] [--topic <text>] <question or decision>"
...
1. Parse `--rounds N` as a total dispatch ceiling (default 5, maximum 15), including
   ...
   `--topic` if supplied to label the conversation on creation.
...
3. Keep debate separate from a required-review claim:
   ...
   "${CLAUDE_PLUGIN_ROOT}/scripts/dispatch.sh" "$THREAD" --read-only --search <<< "$OPENING"
   ```
...
Each dispatch uses the configured model/effort and may incur cost; the ceiling bounds
calls, not dollars. Use available usage/pricing evidence and state an unknown price as
