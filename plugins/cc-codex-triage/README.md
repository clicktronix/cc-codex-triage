...
- `/review [--required --base <ref> --spec <path>] [--lens <name>] [--thread <name>] [--once] [--oneshot] [--cap N] [--model <m>] [--effort <e>] [--background] [--json] [--continue] <paste>` — critique in a review thread; **iterates to APPROVE by default**. `--required` is model-invocable for an owning delivery workflow and produces a machine-readable approval only for the explicit base/spec and exact unchanged clean candidate HEAD/tree/fingerprint. Cap or divergence hard-stop without approval; background/single-pass/JSON/Judge modes never satisfy the required gate. `--once` remains advisory.
...
- `<name>.candidate`, `<name>.review-state`, `<name>.approved` — exact-candidate required-review state. Validate with `review-state.sh check`.
