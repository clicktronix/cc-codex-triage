...
- `/review [--required --base <ref> --spec <path>] [--lens <name>] [--thread <name>] [--once] [--oneshot] [--cap N] [--model <m>] [--effort <e>] [--background] [--json] [--continue] <paste>` — critique in a review thread; advisory mode **iterates to APPROVE by default**. `--required` is model-invocable for an owning delivery workflow and claims one foreground round per invocation. Valid blocking findings return to the owner for fix/test/commit and a new candidate; refuted or explicitly deferred findings are re-reviewed on the same immutable candidate, without a fake change. The first required round pins base/spec/cap until an explicit thread reset or a different thread. It produces a machine-readable approval only for the explicit base/spec and exact unchanged clean candidate HEAD/tree/fingerprint. Cap or divergence hard-stop without approval; background/single-pass/JSON/Judge modes never satisfy the required gate. `--once` remains advisory.
...
- `/status` — one-screen, read-only view: branch, working tree, shared state path, armed gates with their **cycle state**, last verdict per thread, required-review gate state (including a live or expired claim and a
  `CAP_REACHED` hard stop), and the Codex CLI version.
- `/cleanup [--apply] [--older-than <days>]` — find stale/pre-0.5 armed gates, orphan thread logs, stale last-error diagnostics, and — with `--older-than <days>` — whole dormant threads; dry-run by default, `--apply` **archives** them (never deletes, reversible). Safety rails apply to every class: threads with a live dispatch lease, a fresh coherent required-review claim, or an armed gate are never touched, and generic `review`/`plan` threads are listed but never auto-archived. Apply fails closed if any worktree gate cannot be discovered; each final rail re-check and move runs under the dispatch/review and gate-writer mutexes.
...
- `/review` is model-invocable so an owning workflow can complete its declared review gate. Other slash commands stay user-only. Skill `codex-second-opinion` remains a separate single bounded dispatch for a genuine fork and never touches a gate thread.
...
- Same-thread dispatches are serialized by the driver: a second dispatch while one is in flight is refused (exit 10, PID lease + acquisition mutex) — it cannot race the counters/log. Use one gate-owning Claude Code session per worktree and one task per thread. Separate worktrees have isolated armed gates; sessions that deliberately reuse the same thread still share its ledger and review state.

## Where state lives

- `<git-common-dir>/cc-codex-triage/threads/<name>.id` — saved Codex session UUID, shared across worktrees.
- `<git-common-dir>/cc-codex-triage/threads/<name>.log` — append-only prompt/reply audit log.
- `<name>.candidate`, `<name>.review-state`, `<name>.review-loop`, `<name>.approved` — exact-candidate required-review state. Validate with `review-state.sh check`.
- `<absolute-git-dir>/cc-codex-triage/gates/` — current-worktree armed gates and fingerprint cache; never shared with another worktree.
- `~/.codex/sessions/rollout-*.jsonl` — Codex's own rollout files (managed by Codex CLI).

`scripts/state-dir.sh` reports the actual location and migrates legacy `.claude/codex-threads/` state on first write. State therefore survives removal of a disposable worktree.

The plugin never deletes Codex's rollout files. `/thread-new` clears only this repository's pointer, counters, task metadata, and required-review sidecars.
