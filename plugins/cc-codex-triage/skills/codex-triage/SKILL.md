...
| Background dispatch reaped by the harness | A dispatch launched via `Bash(..., run_in_background: true)` dies mid-flight when the harness kills the process group | Use the driver's `--detach` instead — a plain foreground call that re-execs the dispatch in its own session (survives group kills) and prints `DETACHED pid=<pid> output=<thread>.detach-output log-offset=<B>`. Then launch the bundled watcher as a Claude-managed background task — `Bash(bash <plugin>/scripts/detach-watch.sh <thread> <pid> <B>, run_in_background: true)` — its completion notification delivers the reply (exit 0) or the failure diagnostics (exit 1; the worker's real exit status comes from `<thread>.detach-status`, so a post-READY failure that appends nothing to the log is still surfaced). Log polling alone cannot see that failure class — use it only as the fallback when the watcher was reaped (the worker is unaffected). |
