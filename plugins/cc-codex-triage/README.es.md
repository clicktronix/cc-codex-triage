...
- `/review [--required --base <ref> --spec <path>] [--lens <name>] [--thread <name>] [--once] [--oneshot] [--cap N] [--model <m>] [--effort <e>] [--background] [--json] [--continue] <texto>` — crítica que **itera hasta APPROVE por defecto**. `--required` puede ser invocado por el modelo y solo aprueba el base/spec explícito y el HEAD/tree/fingerprint exacto de un candidato limpio sin cambios. Cap o divergencia detienen el flujo sin aprobación; background/single-pass/JSON/Judge nunca satisfacen la verja obligatoria.
...
- `/status` — vista de solo lectura: rama, árbol de trabajo, ruta de estado compartida, verjas armadas y su **estado de ciclo**, último veredicto por hilo y versión de Codex CLI.
...
- `<name>.candidate`, `<name>.review-state`, `<name>.approved` — estado legible por máquina de la revisión obligatoria enlazado al candidato exacto.
