...
- `/review [--required --base <ref> --spec <path>] [--lens <name>] [--thread <name>] [--once] [--oneshot] [--cap N] [--model <m>] [--effort <e>] [--background] [--json] [--continue] <texto>` — crítica que **itera hasta APPROVE por defecto**. `--required` puede ser invocado por el modelo: cada llamada reclama una ronda foreground y devuelve `REQUEST_CHANGES` al workflow propietario para fix/test/commit. Solo aprueba el base/spec explícito y el HEAD/tree/fingerprint exacto de un candidato limpio sin cambios. Cap o divergencia detienen el flujo sin aprobación; background/single-pass/JSON/Judge nunca satisfacen la verja obligatoria.
